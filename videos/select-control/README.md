...
The delivery is `renders/ai-translator-select-control-4k60.mp4`: native 3840x2160, 60 fps, software H.264 at CRF 15. Each frame is captured as a lossless PNG before encoding. Adam narration is generated locally with Kokoro at 1.15x. The original music bed is also generated locally. No paid media or cloud rendering is used.
...
```sh
node build.mjs
hyperframes check . --samples 34 --at-transitions --max-transition-samples 160
python3 tools/render_hq.py --self-check
python3 tools/render_hq.py
```
...
Translations are examples authored for the film. They are not a measured model benchmark. The source app remains unchanged. The extension is free; Google API charges may apply. No Chrome Web Store listing is claimed.
...
The visual direction uses the pacing and product focus of [Apple's September 2025 event](https://www.apple.com/apple-events/), with this extension's own colors and controls. Setup refers to [Google's Gemini API key guide](https://ai.google.dev/gemini-api/docs/api-key). No Apple or Google video assets are included.
